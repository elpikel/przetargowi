defmodule Przetargowi.Reports.ReportGenerator do
  @moduledoc """
  Generates tender report content including statistics, HTML, and graphs.
  """

  alias Przetargowi.Reports.GraphGenerator
  ...
  defp calculate_statistics(tenders) do
    ...
    top_tenders =
      tenders
      |> Enum.map(fn tender ->
        contractor_count = if is_list(tender.contractors), do: length(tender.contractors), else: 0

        details_count =
          if is_list(tender.contractors_contract_details),
            do: length(tender.contractors_contract_details),
            else: 0
  ...
  defp generate_upsell(_region, _order_type) do
    ...
    <div class="upsell-content">
      ...
      <p>Z naszą platformą <strong>Przetargowi</strong> możesz:</p>
      <ul>
  ...
  defp generate_meta_description("detailed", region, order_type, month, stats) do
    ...
    String.slice(
      "Raport przetargów: #{category} w #{region_name} za #{month_name}. Analiza #{total} ogłoszeń.",
      0..159
    )
  end
  ...
  defp generate_meta_description("region_summary", region, _order_type, month, stats) do
    ...
    String.slice(
      "Raport przetargów: region #{region_name} za #{month_name}. Analiza #{total} ogłoszeń.",
      0..159
    )
  end
  ...
  defp generate_meta_description("industry_summary", _region, order_type, month, stats) do
    ...
    String.slice(
      "Raport przetargów: #{category} za #{month_name}. Analiza #{total} ogłoszeń.",
      0..159
    )
  end
  ...
  defp generate_meta_description("overall", _region, _order_type, month, stats) do
    ...
    String.slice(
      "Raport przetargów za #{month_name}. Analiza #{total} ogłoszeń z całej Polski.",
      0..159
    )
  end
