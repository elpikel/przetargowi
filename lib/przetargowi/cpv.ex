defmodule Przetargowi.Cpv do
  ...
  @entries [
    ...
    {"30", "30000000-9", "Maszyny biurowe i liczące, sprzęt i materiały, z wyjątkiem mebli i pakietów oprogramowania"},
    ...
    {"32", "32000000-3", "Sprzęt radiowy, telewizyjny, komunikacyjny, telekomunikacyjny i podobny"},
    ...
    {"37", "37000000-8", "Instrumenty muzyczne, artykuły sportowe, gry, zabawki, wyroby rzemieślnicze"},
    {"374", "37400000-2", "Artykuły i sprzęt sportowy"},
    {"38", "38000000-5", "Sprzęt laboratoryjny, optyczny i precyzyjny (z wyjątkiem szklanego)"},
    {"39", "39000000-2", "Meble (włącznie z biurowymi), wyposażenie, urządzenia domowe i środki czyszczące"},
    ...
    {"452", "45200000-9", "Roboty budowlane w zakresie wznoszenia kompletnych obiektów budowlanych lub ich części oraz roboty w zakresie inżynierii lądowej i wodnej"},
    ...
    {"63", "63000000-9", "Usługi dodatkowe i pomocnicze w zakresie transportu, usługi biur podróży"},
    ...
    {"72", "72000000-5", "Usługi informatyczne: konsultacyjne, opracowywania oprogramowania, internetowe i wsparcia"},
    ...
    {"73", "73000000-2", "Usługi badawcze i eksperymentalno-rozwojowe oraz pokrewne usługi doradcze"},
    ...
    {"79", "79000000-4", "Usługi biznesowe: prawnicze, marketingowe, konsultingowe, rekrutacji, drukowania i zabezpieczania"},
    ...
    {"90", "90000000-7", "Usługi odbioru ścieków, usuwania odpadów, czyszczenia/sprzątania i usługi ekologiczne"},
    {"905", "90500000-2", "Usługi związane z odpadami"},
    {"906", "90600000-3", "Usługi sprzątania oraz usługi sanitarne na obszarach miejskich lub wiejskich"},
  ...
  def search(query, limit) do
    ...
    |> Enum.filter(fn {prefix, full_code, description} ->
      code_match = digits != "" and (String.starts_with?(prefix, digits) or String.starts_with?(full_code, digits))
